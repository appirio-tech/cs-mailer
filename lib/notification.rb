  ...
  private	
    ...
    def self.discussion_board_post(id)
      ...
      comment = query_salesforce("select Id, Name, Member__c, Member__r.name, Member__r.Profile_Pic__c,
        Comment__c, Reply_To__c from Challenge_Comment__c where id = '"+mail.challenge_comment+"' limit 1").first		
...
Rails.logger.info participants.to_yaml
      ...
      unique_recipients(recipients).each do |r|
        StreamingMailer.discussion_board_email(r[:email], mail.subject, r[:name], challenge, comment, r[:member_id]).deliver
